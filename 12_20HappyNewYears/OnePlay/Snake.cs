...
namespace _12_20HappyNewYears
{
    public partial class Snake : Form
    {
        ...
        private enum SnakeDirection//управление направлениями пропишем ниже
        {
            Left,
            Right,
            Up,
        ...
        {
            DoubleBuffered = true;
            BackColor = Color.Blue;//фон игры(цвет = черный)
            StartGame();
        }
        ...
        private void StartGame()
        {
            GenerateFood();
            InitializeSnake();
            isGameEnded = false;//признак окончания игры
            TimerGameLoop.Start();// игровой таймер
            TimerGameLoop.Interval = INITIAL_SNAKE_SPEED_INTERVAL;// значение интервала для таймера в исходном значении константы
        }
        ...
        private void GameOver()
        {
            isGameEnded = true;
            TimerGameLoop.Stop();
            if (MessageBox.Show("Поздравляю - конец игры! Начать заново?", "Конец игры", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                StartGame();
            }
        }
